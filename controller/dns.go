...
func (form DnsForm) ValidateDnsForm() error {
...
func AddDns(c *gin.Context) {
	...
	dnsType, _ := strconv.Atoi(c.PostForm("dns_type"))
	...
	err := dnsForm.ValidateDnsForm()
	if err != nil {
		response.Error(c, 4000, response.ConvertValidationErrorToString(err))
		return
	}
	result := service.AddDnsService(userId, name, path, dnsType, dnsServer, teamId, groupId, policyId, monitorRegion)
	if result != "" {
...
func UpdateDns(c *gin.Context) {
	dnsId, _ := strconv.ParseInt(c.PostForm("dns_id"), 10, 64)
	name := c.PostForm("name")
	path := c.PostForm("path")
	dnsType, _ := strconv.Atoi(c.PostForm("dns_type"))
	dnsServer := c.PostForm("dns_server")
	...
	models.UpdateDns(dnsId, name, path, dnsType, dnsServer, policyId, monitorRegion)
	response.Success(c, 200, "")
}
...
func GetDnsList(c *gin.Context) {
	...
	result := service.GetDnsListService(userId, groupId, page, pageSize)
	response.Success(c, 200, result)
}

func GetFiledDnsList(c *gin.Context) {
	userIdInt := c.GetInt64("userId")
	page, _ := strconv.Atoi(c.Query("page"))
	pageSize, _ := strconv.Atoi(c.Query("page_size"))
	result := service.GetFiledDnsListService(userIdInt, page, pageSize)
	response.Success(c, 200, result)
}

func FileDns(c *gin.Context) {
	id, _ := strconv.ParseInt(c.PostForm("id"), 10, 64)
	models.FileDevice(id, 5)
	response.Success(c, 200, "")
}

func StartMonitorDns(c *gin.Context) {
	id, _ := strconv.ParseInt(c.PostForm("id"), 10, 64)
	models.StartMonitor(id, 5)
	response.Success(c, 200, "")
}

func PauseMonitorDns(c *gin.Context) {
	id, _ := strconv.ParseInt(c.PostForm("id"), 10, 64)
	models.PauseMonitor(id, 5)
	response.Success(c, 200, "")
}

func DeleteDns(c *gin.Context) {
	apiId, _ := strconv.ParseInt(c.Query("dns_id"), 10, 64)
	models.DeleteDevice(apiId, 5)
	response.Success(c, 200, "")
}
